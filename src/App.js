...
class App extends Component {
  ...
  delStringStorage = (date, sum, category) => {
  	let storageState = localStorage.getItem('state'); 
  	storageState = JSON.parse(storageState); // описание 1:47 дитофон
  	let arr = storageState.transactions.filter(item => {
  		return item.sum !== sum && item.category !== category && item.date !== date;
  	});
          ...
          <Table>
            <tbody>
              {transactions
                ...
                .map(({date, sum, category}) => (
                  <tr>
                    <td>{date}</td>
                    <td>{sum} руб</td>
                    <td>{category}</td>
                    <td onClick={this.delStringStorage.bind(this, date, sum, category)}>—</td>
